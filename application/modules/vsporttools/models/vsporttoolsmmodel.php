...
class vsporttoolsmmodel extends CI_Model 
{
    ...
    public function sportToolsDataTable()
    {
        ...
        $this->datatables->select('`Item#`,IndyID,`Man#`,Status, 
                                   ...
                                   TRIM(TRAILING "." FROM(ifnull(CAST(TRIM(TRAILING "0" FROM n_HeightInInches)as CHAR),""))) as n_HeightInInches,
                                   TRIM(TRAILING "." FROM(ifnull(CAST(TRIM(TRAILING "0" FROM n_WidthInInches)as CHAR),""))) as n_WidthInInches,
                                   t_ArtReceivedBy,ti_UploadComplete,
