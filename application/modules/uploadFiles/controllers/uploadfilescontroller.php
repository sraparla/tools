...
class UploadFilesController extends MX_Controller
{
    ...
    public function updateFilesError($uploadID=null,$fileSize=null,$fileName=null)
    {
        $config = Array(
            'protocol' => 'smtp',
        ...
        $body       .= "<p><strong>Error Info:</strong> ".$fileSize. "  ".$fileName."</p>";
    ...
    public function websiteUploadFiles()
    {
        ...
        */
        ...
        //$targetDir = 'uploads';
         // Get parameters
        // Get a file name
        $logID = $this->input->post('uploadFrmID');
        log_message('info', $logID.' The customer is inside uploadFiles controller.');
        ...
        if (isset($_REQUEST["name"])) {
                ...
                log_message('info', '$_Files[file][name] is defined.');
        } else {
                $fileName = uniqid("file_");
                log_message('info', '$_Files[file][name] is not defined.');
        }
        ...
        if(!empty($_FILES))
        {
            if($_FILES["file"])
            {
                 if($_FILES["file"]["size"])
                 {
                     ...
                     log_message('info', '$_Files[file][size] is defined.');
                 }
                 else
                 {
                     $fileSize="index is set but not size not set";
                     log_message('info', '$_Files[file][size] is not defined.');
                 }    
            }
            else
            {
                $fileSize="index not set";
                log_message('info', '$_Files[file] is not defined.');
        ...
        {
            $fileSize="not set";
            log_message('info', '$_Files is empty.');
        ...
        echo "$insertedUploadID: ".$insertedUploadID;
        if(!isset($insertedUploadID) || empty($insertedUploadID) || $insertedUploadID == "0" || $insertedUploadID == "")
        {
            $insertedUploadID = "insertedID not set";
            log_message('info', 'uploadID is not defined.');
            
        }    
       
        
        if($insertedUploadID == "insertedID not set")
        {
            //send an email with the info
            log_message('info', 'sending uploadFiles error start');
            $this->updateFilesError($insertedUploadID,$fileSize,$fileName);
            log_message('info', 'sending uploadFiles error done');
            exit(1);
        }
